...
$(function () {
    if (id !== null) {
        $.get("/table_utils/info", {borrowMoneyId: id, "table_utils.tableName": "borrow_money"}, function (data) {
            console.log(data["content"]["0"]);
            // 铺数据
            showData4Object(data["content"]["0"]);
            convertNum(data["content"]["0"]["borrowMoneyMoney"], "innerTable");
            // 日期格式转换
            let borrowMoneyDate = $(".borrowMoneyDate");
            let tempVal = borrowMoneyDate.text();
            tempVal = tempVal.substring(0,10);
            borrowMoneyDate.text(tempVal);
        }, "json")
    }
